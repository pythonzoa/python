...
def fetch_news_by_titles(keywords, max_results_per_keyword=1):
    ...
    for keyword in keywords:
        ...
        url = f"https://www.google.com/search?q={keyword}&tbm=nws&when=1d"
        ...
        for a in soup.find_all('a', href=True):
            ...
            if '/url?esrc=' in a['href']:
                ...
                if real_url:
                    ...
                    news_data['URL'].append(f'=HYPERLINK("{real_url}", "link")')
    ...
    excel_filename = f'{today}_Strong_News.xlsx'
    with pd.ExcelWriter(excel_filename, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name='News', index=False)
        worksheet = writer.sheets['News']
        # Excel에서 하이퍼링크가 동작하도록 URL 컬럼의 셀 포맷 변경
        for idx, val in enumerate(df['URL']):
            worksheet.write_formula(idx+1, 2, val)

    print(f"Saved to {excel_filename}")
# 함수 호출
fetch_news_by_titles(first_elements, 1)
...
def do_something():
    ...
    latest_folder_path = find_latest_folder(r"D:\pythonProject21\mailing")  # 검색할 디렉토리 경로
