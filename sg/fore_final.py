...
stock_cap = krx_list[krx_list['Marcap'] >= 3e12][['Code', 'Name']]
...
for stock_code in random_stock_codes:
    ...
    # df 데이터프레임에서 '외국인합계' 열이 존재하는지 확인
    if '외국인합계' in df.columns:
        # '외국인합계' 열이 존재하면, 52주 최고값 계산
        ...
        # '외국인합계' 열이 없을 경우, 사용자에게 알림
        print(f"'외국인합계' 열이 {stock_code} 데이터프레임에 존재하지 않습니다.")
        df['is_high'] = 0  # '외국인합계' 열이 없는 경우 'is_high' 열을 0으로 설정
        df['외국인합계'] = 0  # '외국인합계' 열이 없는 경우 '외국인합계' 열을 0으로 설정
...
output_df.to_excel('high_returns_all_fore.xlsx', index=True)
# high_df.to_excel('high_returns_all_fore.xlsx', index=True)
