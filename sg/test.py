...
def process_market_data(stock_cap, market_name):
    ...
    for stock_code in random_stock_codes:
        try:
            ...
            # KRX 데이터를 통해 투신 순매수량 가져오기
            df = stock.get_market_trading_value_by_date(start_date, end_date, stock_code, detail=True)
            ...
            if '투신' in df.columns:
                df['52w_high'] = df['투신'].rolling(window=224, min_periods=1).max()
                df['52w_high'] = df['52w_high'].apply(lambda x: x if x > 0 else None)
                df['is_high'] = (df['투신'] == df['52w_high']).astype(int)
            else:
                print(f"'투신' 열이 {stock_code} 데이터프레임에 존재하지 않습니다.")
                df['is_high'] = 0
                df['투신'] = 0
            ...
            merged_df = pd.merge(price_df, df[['투신', 'is_high', '시가총액']], left_index=True, right_index=True,
                                 how='left')
    ...
    if 'is_high' in merged_all_df.columns:
        ...
        if market_name == '코스피':
            ...
            high_df['시가총액_구간'] = pd.qcut(high_df['시가총액'], 10, labels=[f'{i+1}구간' for i in range(10)], duplicates='drop')
...
with pd.ExcelWriter(f'stockfore{today}ts.xlsx') as writer:
    if kospi_results:
