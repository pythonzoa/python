...
def process_market_data(stock_cap, market_name):
    ...
    for stock_code in random_stock_codes:
        try:
            ...
            if '사모' in df.columns:
                df['52w_high'] = df['사모'].rolling(window=224, min_periods=1).max()
                df['52w_high'] = df['52w_high'].apply(lambda x: x if x > 0 else None)
                df['is_high'] = (df['사모'] == df['52w_high']).astype(int)
            else:
                print(f"'사모' 열이 {stock_code} 데이터프레임에 존재하지 않습니다.")
                df['is_high'] = 0
                df['사모'] = 0
            ...
            merged_df = pd.merge(price_df, df[['사모', 'is_high', '시가총액']], left_index=True, right_index=True,
                                 how='left')
...
with pd.ExcelWriter(f'stockfore{today}sm.xlsx') as writer:
    if kospi_results:
