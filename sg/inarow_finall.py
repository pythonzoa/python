...
for stock_code in random_stock_codes:
    ...
    df['positive_buy'] = (df['기관합계'] > 0).astype(int)
    ...
    # 데이터 병합
    merged_df = pd.merge(price_df, df[['기관합계', 'continuous_buy_days','buy_days_until_break']], left_index=True, right_index=True, how='left')
